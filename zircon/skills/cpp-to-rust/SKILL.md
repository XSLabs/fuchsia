...
1.  **Define Coder Agent (`cpp-to-rust-coder`)**:
    ...
    * **System Prompt**: Focuses on implementing the Rust port following Zircon
      patterns defined in `zircon/skills/cpp-to-rust-rubric/SKILL.md` (and
      `zircon/skills/cpp-to-rust-dispatcher/SKILL.md` for dispatcher ports).
2.  **Define Reviewer Agent (`cpp-to-rust-reviewer`)**:
    * **Name**: `cpp-to-rust-reviewer`
    * **System Prompt**: Focuses on reviewing the Rust port against the C++ code
      and evaluation criteria defined in
      `zircon/skills/cpp-to-rust-rubric/SKILL.md` (and
      `zircon/skills/cpp-to-rust-dispatcher/SKILL.md` for dispatcher ports),
      checking for API parity, memory safety, test coverage, and fuzz test
      parity.
  ...
  "Subagents": [
    {
      ...
      "Workspace": "inherit",
      "Prompt": "Please read `zircon/skills/cpp-to-rust-rubric/SKILL.md` (and `zircon/skills/cpp-to-rust-dispatcher/SKILL.md` if porting a dispatcher) and prepare to port C++ code to Rust. Wait for further instructions."
    },
    {
      "Role": "CppToRust Reviewer",
      "TypeName": "cpp-to-rust-reviewer",
      "Workspace": "inherit",
      "Prompt": "Please read `zircon/skills/cpp-to-rust-rubric/SKILL.md` (and `zircon/skills/cpp-to-rust-dispatcher/SKILL.md` if reviewing a dispatcher port) and prepare to review the port. Wait for further instructions."
    }
  ]
}
```
...
> You must follow `zircon/skills/cpp-to-rust-rubric/SKILL.md`.
>
> Relevant files:
...
**Message to Coder:**
> Here is the review report for `<component>` generated by the Reviewer. Please implement the changes described in the "Actionable Instructions" section according to `zircon/skills/cpp-to-rust-rubric/SKILL.md`.
>

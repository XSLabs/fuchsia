...
//! that evaluate to `u32` (e.g. `debug::INFO` or a local variable).
...
#[macro_export]
macro_rules! dprintf {
    (CRITICAL, $($arg:tt)*) => { $crate::dprintf!($crate::CRITICAL, $($arg)*) };
    (ALWAYS, $($arg:tt)*) => { $crate::dprintf!($crate::ALWAYS, $($arg)*) };
    (INFO, $($arg:tt)*) => { $crate::dprintf!($crate::INFO, $($arg)*) };
    (SPEW, $($arg:tt)*) => { $crate::dprintf!($crate::SPEW, $($arg)*) };
    ($level:expr, $($arg:tt)*) => {
        if $crate::dprintf_enabled($level) {
            $crate::print_dprintf_args(core::format_args!($($arg)*));
        }
    };
}

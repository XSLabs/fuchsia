...
pub unsafe extern "C" fn apic_io_init(
    ...
    // SAFETY: `io_apic_descs` points to a valid array of size `num_io_apic_descs` provided by early boot.
    let io_apic_descs = unsafe { core::slice::from_raw_parts(io_apic_descs, num_io_apic_descs) };
    // SAFETY: `overrides` points to a valid array of size `num_overrides` provided by early boot.
    let overrides = unsafe { core::slice::from_raw_parts(overrides, num_overrides) };

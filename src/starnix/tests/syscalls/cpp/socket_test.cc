...
TEST(UnixSocket, ScmRightsTruncation) {
  ...
  memcpy(CMSG_DATA(cmsg), send_fds, kSendFdCount * sizeof(int));
  ASSERT_EQ(1, sendmsg(sv[0], &send_msg, 0));
  ...
  ASSERT_EQ(1, recvmsg(sv[1], &recv_msg, 0));

  EXPECT_NE(0, recv_msg.msg_flags & MSG_CTRUNC);

  int new_fds = 0;
  for (int fd = max_fd_before + 1; fd < 256; fd++) {
    if (fcntl(fd, F_GETFD) != -1) {
      new_fds++;
      close(fd);
    }
...
TEST_P(CmsgAlignmentTest, ScmRightsFollowedByCredentials) {
  ...
  int one = 1;
  ASSERT_EQ(0, setsockopt(sv[1], SOL_SOCKET, SO_PASSCRED, &one, sizeof(one)));
  ...
  for (int i = 0; i < num_fds; i++) {
    memcpy(CMSG_DATA(cmsg) + i * sizeof(int), &pipes[i][0], sizeof(int));
  }
  ASSERT_EQ(1, sendmsg(sv[0], &send_msg, 0));
  ...
  for (struct cmsghdr* c = CMSG_FIRSTHDR(&recv_msg); c; c = CMSG_NXTHDR(&recv_msg, c)) {
    ...
    if (c->cmsg_level == SOL_SOCKET && c->cmsg_type == SCM_RIGHTS) {
      ...
      EXPECT_EQ(received_fds, num_fds);
      for (int i = 0; i < received_fds; i++) {
        int recv_fd;
        memcpy(&recv_fd, CMSG_DATA(c) + i * sizeof(int), sizeof(int));
        EXPECT_GE(recv_fd, 0);
        close(recv_fd);
      }
    }
